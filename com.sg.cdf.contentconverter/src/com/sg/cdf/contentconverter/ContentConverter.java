...
//		loadService();
	...
	// String sTaskQueueTimeout = props
	//				.getProperty("office.task.queuetimeout"); //$NON-NLS-1$
	// String sMaxTasksPerProcess = props
	//				.getProperty("office.task.maxperprocess"); //$NON-NLS-1$
	//
...
//		}
	...
	public static void convert(File inputFile, File outputFile,
			String mediaType, String sourceMediaType) throws Exception {
		if ("office".equals(sourceMediaType)) {
			// if (officeManager == null) {
			// loadService();
			// }
		...
		if ("dwg".equals(sourceMediaType)) {
			DWGConverter dwg = new DWGConverter(mediaType);
			dwg.convert(inputFile, outputFile);
		}
	}
